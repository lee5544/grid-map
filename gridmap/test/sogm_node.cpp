 ...
 * @LastEditTime: 2024-10-22 16:55:18
 * @Description:
 * @Subscriber:
 * @Publisher:
 */
...
void publishSlideGlobalGridMapRange()
{
    ...
    map_min_pos = map_.VoxelToWorld(map_.getOrigin());
    map_max_pos = map_.VoxelToWorld(map_.getOrigin()) + map_.VoxelToWorld(map_.getNum3dim());
    // map_min_pos = local_map_boundary_min_;
    // map_max_pos = local_map_boundary_max_;
...
void publishNewOcc()
{
    ...
    {
        pos = map_.IndexToWorld(newOcc->at(i));
...
void publishNewFree()
{
    ...
    {
        pos = map_.IndexToWorld(newFree->at(i));
...
void publishSlideGlobalOccMap()
{
    ...
    for (int x = min_cut(0); x <= max_cut(0); ++x)
        for (int y = min_cut(1); y <= max_cut(1); ++y)
            ...
            {
                index = map_.VoxelToIndex(Eigen::Vector3i(x, y, z));
                if (!map_.isOccupied(index))
                    continue;

                pos = map_.IndexToWorld(index);
